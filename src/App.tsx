...
const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-900 text-white font-sans overflow-hidden">
      <header className="p-4 bg-gray-800 border-b border-gray-700 flex justify-between items-center z-10">
        <div>
          <h1 className="text-xl font-bold text-blue-400">
            Arjuna's Network Graph Visualiser
          </h1>
        </div>
      ...
      {/* 2. Removed <Grid> and <Row> here. <main> is now a direct child. */}
      <main ref={containerRef} className="flex-1 relative bg-slate-950 w-full">
        {error && (
        ...
        {/* 3. Pass the dimensions to the ForceGraph so it fills the <main> container exactly */}
        ...
        {selectedNode && (
          <div className="absolute top-4 right-4 w-80 bg-gray-800/95 backdrop-blur-sm border border-gray-600 p-4 rounded-lg shadow-xl z-20 animate-in fade-in slide-in-from-right-4 text-left">
            <div className="flex justify-between items-center mb-3 border-b border-gray-700 pb-2">
              <h2 className="font-bold text-blue-300 text-lg m-0">
            ...
            <div className="flex flex-col gap-2 max-h-[60vh] overflow-y-auto pr-1">
              {Object.entries(selectedNode).map(([key, value]) => {
                ...
                return (
                  <div
                    key={key}
                    className="bg-black/30 p-2 rounded text-sm text-left"
                  >
                    <span className="block text-xs font-semibold text-gray-400 capitalize mb-0.5">
                      {cleanKey}
                    </span>
      ...
      <footer className="p-2 bg-gray-800 border-t border-gray-700 text-[10px] text-center text-gray-500 uppercase tracking-tighter">
        Thank you for tying out my little program 
      </footer>
    </div>
  );
}
